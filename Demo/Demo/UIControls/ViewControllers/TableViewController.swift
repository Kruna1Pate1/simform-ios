...
class TableViewController: UIViewController {
    ...
    private var languages: [LanguageModel] = LanguageModel.dummyLanguages()
        ...
        if settings[indexPath.section].key == "Device info" {
            if let cell = tableView.dequeueReusableCell(withIdentifier: "DescriptionCell", for: indexPath) as? DescriptionCell {
                ...
                cell.expand = { isExpanded in
                    // tableView.reloadRows(at: [indexPath], with: .automatic) - don't reload rows properly
...
//        return UITableView.automaticDimension - returs dynamic height
        if settings[indexPath.section].key == "Device info" {
...
extension TableViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == 1 {
            let header = UIView(frame: CGRect(x: 0, y: 0, width: Int(tableView.frame.width - 20), height: 30))
